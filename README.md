...
now let's put the order in the code so that all the variables are at the top, and secondly, in order not to constantly manually read the range for each satellite and for each epoch, let's change the program. Let's create a cycle. 4 - 4 epochs (time 0:00-1:30)
</p>
